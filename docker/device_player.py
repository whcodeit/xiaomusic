...
class XiaoMusicDevice:
    ...
    async def _check_and_download_music(self, name, search_key, allow_download):
        ...
        # 下载歌曲
        success = await self.download(search_key, name)
        if not success:
            return False
        # 把文件插入到播放列表里
    ...
    async def download(self, search_key, name):
        ...
        if self._download_proc.returncode != 0:
            self.log.error(f"下载失败 {search_key} {name}, returncode: {self._download_proc.returncode}")
            await self.do_tts(f"下载失败{search_key}")
            return False
        chmodfile(file_path)
        return True
